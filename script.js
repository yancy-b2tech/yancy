...
function initTeamMemberModals() {
    ...
    document.querySelectorAll('.team-member').forEach(member => {
        ...
        const open = () => {
            try {
                const name = (member.querySelector('.member-info h4') || {}).textContent || '';
                const title = (member.querySelector('.member-title') || {}).textContent || '';
                const desc = (member.querySelector('.member-description') || {}).textContent || '';
                const imgEl = member.querySelector('.member-avatar img');
                const imgSrc = imgEl ? imgEl.src : '';
                const altText = imgEl ? imgEl.alt : '';

                bodyEl.innerHTML = `
                    <div style="text-align:center;">
                        ${imgSrc ? `<img src="${imgSrc}" alt="${altText}" style="width:120px;height:120px;border-radius:50%;object-fit:cover;border:4px solid #d4af37;margin-bottom:15px;" />` : ''}
                        <h4 style="margin:10px 0;color:#333;">${name}</h4>
                        <p style="margin:6px 0;color:#d4af37;font-weight:600;">${title}</p>
                        <p style="margin-top:10px;color:#666;line-height:1.6;">${desc}</p>
                    </div>
                `;
                showModal();
            } catch (err) {
                console && console.warn && console.warn('打开团队成员弹窗失败:', err);
            }
        };
